...
\section{对红外小目标进行数学建模分析}

在这里分析分什么采用这个深度展开


为了形式化地描述红外小目标检测问题，本文首先从单帧红外图像出发进行建模。
设给定的红外图像记为
\[
...
\begin{itemize}
    \item 背景辐射分量 $B$，刻画云层、地面、海面等大尺度结构；
    \item 目标分量 $T$，对应场景中的红外小目标；
    \item 噪声分量 $N$，包括传感器噪声、大气扰动等随机干扰。
\end{itemize}
...
观测到的红外图像 $\mathbf{D}\in\mathbb{R}^{H\times W}$ 可以本质上分解成两个组成部分
$D = B+ T$
...
`\subsection{红外小目标检测的数学建模分析}  红外成像系统获取的观测图像可以表示为一个二维灰度矩阵
 \[ \mathbf{D} \in \mathbb{R}^{H \times W}, \] 其中 $H$ 与 $W$ 分别为图像的高度和宽度，
 每个像素记录了对应视线方向上的红外辐射强度。 在典型场景中，图像主要包含三部分信息：
 一是大面积、结构相对平滑的背景辐射；二是尺寸较小、亮度略高的目标；三是由成像设备和环境带来的随机噪声。
  因此，可以将观测图像抽象建模为 \begin{equation}     
    \mathbf{D} = \mathbf{B} + \mathbf{T} + \mathbf{N}, \end{equation} 其中，
    $\mathbf{B}$ 表示背景分量，$\mathbf{T}$ 为小目标分量，$\mathbf{N}$ 为噪声项。 
     在后续处理中，噪声 $\mathbf{N}$ 通常通过数据保真项统一建模，因此核心任务可以理解为：
     在给定观测 $\mathbf{D}$ 的情况下，尽可能准确地将背景 $\mathbf{B}$ 与目标 $\mathbf{T}$ 区分开来。
      要实现这一点，需要对背景和目标分别施加合理的先验约束。  \subsubsection{背景与目标先验的抽象建模}  
      红外场景中的背景往往由天空、海面、地表等大尺度结构构成，在空间上具有较强的相关性和冗余性： 
      一方面，相邻像素亮度变化较为平缓；另一方面，如果将多帧图像或一组相似图像块按列或按组堆叠，
      背景部分往往可以由较低维度的子空间近似表示。 这两点表明，背景分量适合用\emph{平滑性}或\emph{低秩性}进行刻画。
        为兼容不同形式的背景先验，这里用一个一般的正则函数 $R(\mathbf{B})$ 抽象表示背景约束，例如：
         \begin{itemize}     
            \item 若强调局部平滑性，可取    
             \[         R(\mathbf{B}) = \|\nabla \mathbf{B}\|_1,     \]     
             即利用总变差（TV）抑制背景的高频细节；     
             \item 若考虑多帧或块堆叠结构，可选用核范数   
               \[         R(\mathbf{B}) = \|\mathbf{B}\|_*,     \]     
               以反映背景矩阵的低秩特性。 \end{itemize}  
               相比之下，小目标的空间分布具有明显的\emph{稀疏性}：一幅图像中真正的目标像素数量远少于背景像素；
                同时，目标通常以小斑块的形式存在，具有一定的局部连通性和亮度对比度。 
                因此，可以用另一个正则函数 $S(\mathbf{T})$ 来表达目标先验，
                其中应包含稀疏约束及必要的空间结构约束，例如：
                 \begin{itemize}    
                     \item 单纯追求稀疏性时，可采用   
                       \[         S(\mathbf{T}) = \|\mathbf{T}\|_1,     \]    
                        促使绝大部分像素在目标分量中为零；   
                          \item 若希望消除孤立噪点并保留小目标的连通形态，可进一步加入总变差项     \[         S(\mathbf{T}) = \|\mathbf{T}\|_1 + \beta\,\|\nabla \mathbf{T}\|_1,     \]     其中 $\beta>0$ 用于控制空间平滑约束强度。 \end{itemize}  \subsubsection{统一优化模型与能量函数构建}  在上述背景和目标先验的基础上，可以将红外小目标检测建模为一个分解问题：在观测 $\mathbf{D}$ 的约束下，寻找合适的背景 $\mathbf{B}$ 与目标 $\mathbf{T}$，使得二者既符合各自先验，又能共同解释观测数据。该问题可以写成如下优化形式： \begin{equation}     \min_{\mathbf{B},\,\mathbf{T}} \; R(\mathbf{B}) + \lambda\,S(\mathbf{T})     \quad \text{s.t.} \quad     \mathbf{D} = \mathbf{B} + \mathbf{T},     \label{eq:constraint_model} \end{equation} 其中 $\lambda>0$ 为平衡背景先验与目标先验的重要系数。  直接处理式 \eqref{eq:constraint_model} 中的等式约束会给数值求解带来一定困难。 为此，可采用惩罚思想将约束纳入目标函数，将模型转化为无约束优化问题： \begin{equation}     \min_{\mathbf{B},\,\mathbf{T}} \;     \mathcal{J}(\mathbf{B}, \mathbf{T})     = R(\mathbf{B})     + \lambda\,S(\mathbf{T})     + \frac{\mu}{2}\,\|\mathbf{D} - \mathbf{B} - \mathbf{T}\|_F^2,     \label{eq:unconstrained_model} \end{equation} 其中，$\mu>0$ 为惩罚系数，$\|\cdot\|_F$ 表示 Frobenius 范数。 式 \eqref{eq:unconstrained_model} 的三项分别对应背景先验、目标先验和数据保真项，其物理意义可以理解为：在尽量贴合观测图像的前提下，使背景部分尽量“规则”（低秩或平滑），同时使目标部分尽量“稀疏且紧凑”。  \subsubsection{交替优化与子问题形式}  针对式 \eqref{eq:unconstrained_model}，常用的求解思路是采用交替优化策略，即在每一步中固定一个变量并更新另一个变量，从而将原问题分解为两个相对简单的子问题。  首先，在目标 $\mathbf{T}$ 固定的情况下，关于背景 $\mathbf{B}$ 的子问题为 \begin{equation}     \mathbf{B}^{k}     = \arg\min_{\mathbf{B}}     \left\{ R(\mathbf{B})     + \frac{\mu}{2}\,\|\mathbf{D} - \mathbf{B} - \mathbf{T}^{k-1}\|_F^2 \right\}.     \label{eq:B_subproblem} \end{equation} 该子问题可以视为对正则项 $R(\mathbf{B})$ 的一次近端更新： 给定当前的“残差”图像 $\mathbf{D} - \mathbf{T}^{k-1}$，在背景先验的约束下寻找一个最合理的背景估计 $\mathbf{B}^{k}$。 当 $R(\mathbf{B})$ 选取为核范数或总变差时，对应的近端算子分别与奇异值软阈值或 TV 去噪有关。  其次，在背景 $\mathbf{B}$ 固定时，关于目标 $\mathbf{T}$ 的子问题为 \begin{equation}     \mathbf{T}^{k}     = \arg\min_{\mathbf{T}}     \left\{ \lambda\,S(\mathbf{T})     + \frac{\mu}{2}\,\|\mathbf{D} - \mathbf{B}^{k} - \mathbf{T}\|_F^2 \right\}.     \label{eq:T_subproblem} \end{equation} 这一子问题本质上是对目标分量进行一次“稀疏重建”：在当前的背景估计 $\mathbf{B}^{k}$ 确定后，残差 $\mathbf{D} - \mathbf{B}^{k}$ 中既包含小目标也包含噪声，通过正则项 $S(\mathbf{T})$ 的作用，从中筛选出符合稀疏和局部连通特性的目标响应。  在具体求解时，可以借助近端梯度法或 ADMM 等迭代方案对式 \eqref{eq:B_subproblem} 和式 \eqref{eq:T_subproblem} 分别进行更新，从而构成一个“背景--目标”交替优化框架。经过若干次迭代后，$\mathbf{B}$ 与 $\mathbf{T}$ 将逐步收敛到较稳定的分解结果。  \subsubsection{与特征融合和图像重建的关系}  本文的研究重点是“基于特征融合和图像重建的红外小目标检测”。 在上述数学建模框架下，$\mathbf{B}$ 与 $\mathbf{T}$ 的分解不仅是一个前置的预处理步骤，更是后续特征学习与融合的基础：  一方面，$\mathbf{B}$ 可看作背景的重建结果，其去除了大部分小目标和噪声成分，有利于构造显著性图或对比度特征； 另一方面，$\mathbf{T}$ 中保留了与小目标高度相关的结构信息，可以与深度网络提取的多尺度特征进行融合，进一步提升检测的鲁棒性与精度。 因此，式 \eqref{eq:unconstrained_model} 所对应的分解过程，可以被视为一个\emph{基于先验的图像重建模块}，为后续的特征融合与深度检测网络提供结构清晰、语义明确的输入。  总体来看，上述数学建模将红外小目标检测问题统一框架在“低秩背景 + 稀疏目标 + 数据保真”的能量优化形式之中，为结合深度展开网络、特征融合策略及图像重建模块提供了严谨而灵活的理论基础。`
